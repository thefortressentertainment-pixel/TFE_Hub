...
Doctrine:
  1. ALWAYS use --background for one-shot ops (render, bake, export, batch) — no
     GUI, deterministic, fast. The GUI stays untouched.
  2. For an interactive or long-running session, park a cockpit server, then
     drive it slice-by-slice. One task per request; set `result =` to get data
     back; errors come back as {"err": traceback}.
  3. Never drive Blender's own UI when bpy can do it; bpy is the key.
  4. import bpy works only under the bundled python — use that binary, never
     the Homebrew python.